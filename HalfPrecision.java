...
public class HalfPrecision {
    ...
    private final int expSize = 5;
    private final int mantissaSize = 10;
    private final int hexOutputPrecision = 3; 

    public HalfPrecision(int bits) {
    ...
    private HalfPrecision(int sign, int exp, int mantissa) {
        this.sign = sign;
        this.exp = exp;
    ...
    public HalfPrecision add(HalfPrecision other) {
        ...
        return new HalfPrecision(newSign, newExp, bits.intValue());
    }

    public HalfPrecision subtract(HalfPrecision other) {
        if (isNaN() || other.isNaN() || isInf() && other.isInf() && sign == other.sign) return getNaN();
        if (isInf()) return getInf(sign);
        if (other.isInf()) return getInf(other.sign);
        int e1 = exp + (isSubnormal() ? 1 : 0), e2 = other.exp + (other.isSubnormal() ? 1 : 0);
    ...
    public HalfPrecision multiply(HalfPrecision other) {
    ...
    public HalfPrecision divide(HalfPrecision other) {
    ...
    public String toString() {
        ...
        if (isZero()) return sign == 0 ? "0x0.000p+0" : "-0x0.000p+0";
        int out = mantissa << 2, shift = 0;
        if (isSubnormal()) { shift = Integer.numberOfLeadingZeros(out) - 19; out <<= shift; out -= 1 << 12; }
    ...
    public HalfPrecision getInf(int sign) {
        return new HalfPrecision(sign, (1 << expSize) - 1, 0);
    }
    ...
    public HalfPrecision getNaN() {
        return new HalfPrecision(0, (1 << expSize) - 1, (1 << mantissaSize - 1));
    }
    ...
    public HalfPrecision getZero(int sign) {
        return new HalfPrecision(sign, 0, 0);
    }
